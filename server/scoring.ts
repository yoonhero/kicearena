...
export const effectiveSubmissionPenaltyMs = (room: Pick<RoomState, "startedAt">, submission: PlayerPublic["submissions"][number]) => {
...
export const normalizeSubmissionPenalty = (room: Pick<RoomState, "startedAt">, submission: PlayerPublic["submissions"][number]) => ({
  ...submission,
  penaltyMs: effectiveSubmissionPenaltyMs(room, submission)
});

export const formatPenaltyMinutes = (penaltyMs: number) => Math.max(0, Math.round(penaltyMs / 60000));

export const derivePlayerScoreState = (room: Pick<RoomState, "startedAt">, player: PlayerPublic) => {
...
export const makeStandings = (room: Pick<RoomState, "startedAt"> & { players: Map<string, PlayerPublic> }, players: PlayerPublic[] = [...room.players.values()], visibleUntil: number | null = null): StandingPublic[] =>
  players
    .map((player) => {
