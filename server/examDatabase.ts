...
import type { CaptureQuality, CaptureSummary, ExamManifest, ProblemBodyBlock, ProblemManifest } from "../shared/game.js";
...
const jsonParam = (value: unknown) => (value === undefined || value === null ? null : JSON.stringify(value));
...
export const seedExamCatalog = async (db: ExamCatalogDatabase, exams: ExamManifest[], activeExamIds = ACTIVE_EXAM_IDS, assets: ExamAssetInput[] = []) => {
  ...
  try {
    for (const exam of exams) {
      await db.query(
        `INSERT INTO exams (id, title, subtitle, time_limit_sec, release_at, capture_summary, active, updated_at)
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           updated_at = now()`,
        [exam.id, exam.title, exam.subtitle, exam.timeLimitSec, exam.releaseAt ?? null, jsonParam(exam.captureSummary), activeExamIds.has(exam.id)]
      );
      ...
      await db.query("DELETE FROM problems WHERE exam_id = $1 AND NOT (id = ANY($2::text[]))", [exam.id, exam.problems.map((problem) => problem.id)]);
    }
    ...
    for (const exam of exams) {
      const assetPaths = assets.filter((asset) => asset.examId === exam.id).map((asset) => asset.path);
      await db.query("DELETE FROM exam_assets WHERE exam_id = $1 AND NOT (path = ANY($2::text[]))", [exam.id, assetPaths]);
    }
...
export const composeExamManifests = (examRows: ExamRow[], problemRows: ProblemRow[]): ExamManifest[] => {
...
export const readAdminExamsFromDatabase = async (db: ExamCatalogDatabase): Promise<AdminExamManifest[]> => {
...
export const createExamInDatabase = async (db: ExamCatalogDatabase, input: ExamCreateInput): Promise<AdminExamManifest> => {
  const result = await db.query<AdminExamRow>(
    `INSERT INTO exams (id, title, subtitle, time_limit_sec, active, updated_at)
     VALUES ($1, $2, $3, $4, $5, now())
     RETURNING id, title, subtitle, time_limit_sec, release_at, capture_summary, active`,
    [input.id, input.title, input.subtitle, input.timeLimitSec, input.active]
  );
  const [manifest] = composeExamManifests(result.rows, []);
  return { ...manifest, active: result.rows[0]?.active === true };
};

export const updateExamSettingsInDatabase = async (db: ExamCatalogDatabase, examId: string, update: ExamSettingsUpdateInput): Promise<AdminExamManifest | null> => {
  const result = await db.query<AdminExamRow>(
    `UPDATE exams
     SET title = $2,
         subtitle = $3,
     ...
     WHERE id = $1
     RETURNING id, title, subtitle, time_limit_sec, release_at, capture_summary, active`,
    [examId, update.title, update.subtitle, update.timeLimitSec, update.active, update.releaseAt]
  );
  const [manifest] = composeExamManifests(result.rows, []);
  return manifest ? { ...manifest, active: result.rows[0]?.active === true } : null;
};

export const createProblemInDatabase = async (db: ExamCatalogDatabase, examId: string, input: ProblemCreateInput): Promise<ProblemManifest | null> => {
  const result = await db.query<ProblemRow>(
    `WITH next_problem AS (
     ...
     RETURNING exam_id, id, number, title, answer_kind, answer, difficulty, point_value,
               image, body, text, source_number, source_page, bbox, section, capture_quality`,
    [examId, input.title, input.answerKind, input.answer, input.difficulty, input.pointValue, jsonParam(input.body)]
  );
  const [manifest] = composeExamManifests(
    [{ id: examId, title: "", subtitle: "", time_limit_sec: 1, release_at: null, capture_summary: null }],
    result.rows
  );
  return manifest?.problems[0] ?? null;
};

export const saveExamAssetInDatabase = async (db: ExamCatalogDatabase, asset: ExamAssetInput): Promise<ExamAsset | null> => {
  const result = await db.query<{ exam_id: string; path: string; content_type: string; body: Buffer; updated_at: Date }>(
    `INSERT INTO exam_assets (exam_id, path, content_type, body, updated_at)
     SELECT $1, $2, $3, $4, now()
     WHERE EXISTS (SELECT 1 FROM exams WHERE id = $1)
     ON CONFLICT (exam_id, path) DO UPDATE SET
  ...
  return row ? { examId: row.exam_id, path: row.path, contentType: row.content_type, body: row.body, updatedAt: row.updated_at } : null;
};

export const updateProblemInDatabase = async (db: ExamCatalogDatabase, examId: string, problemId: string, update: ProblemUpdateInput): Promise<ProblemManifest | null> => {
  const result = await db.query<ProblemRow>(
    `UPDATE problems
     SET title = $3,
         answer_kind = $4,
     ...
     RETURNING exam_id, id, number, title, answer_kind, answer, difficulty, point_value,
               image, body, text, source_number, source_page, bbox, section, capture_quality`,
    [examId, problemId, update.title, update.answerKind, update.answer, update.difficulty, update.pointValue, jsonParam(update.body)]
  );
  const [manifest] = composeExamManifests(
    [{ id: examId, title: "", subtitle: "", time_limit_sec: 1, release_at: null, capture_summary: null }],
    result.rows
...
export const readExamAssetFromDatabase = async (db: ExamCatalogDatabase, examId: string, assetPath: string, requireActive = true): Promise<ExamAsset | null> => {
  const result = await db.query<{ exam_id: string; path: string; content_type: string; body: Buffer; updated_at: Date }>(
