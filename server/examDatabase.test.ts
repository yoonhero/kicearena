...
describe("exam database row mapping", () => {
  ...
  it("saves admin uploaded assets with an exam existence guard", async () => {
    ...
    expect(queries[0]?.values).toEqual(["db-exam", "diagrams/uploaded.svg", "image/svg+xml; charset=utf-8", body]);
  });
});
...
describePostgres("postgres exam catalog storage", () => {
  ...
  it("migrates, seeds, reads only active exam problems, and removes stale problems", async () => {
    ...
    await seedExamCatalog(pool, [exam, { ...exam, id: "inactive-exam", title: "Inactive" }], new Set(["db-exam"]), [
      {
        examId: "db-exam",
    ...
    expect(adminInitial.find((candidate) => candidate.id === "inactive-exam")).toMatchObject({ active: false });
    ...
    await expect(readExamAssetFromDatabase(pool, "db-exam", "diagrams/graph.svg")).resolves.toMatchObject({
      examId: "db-exam",
      path: "diagrams/graph.svg",
      contentType: "image/svg+xml; charset=utf-8",
      body: Buffer.from("<svg><path d=\"M0 0\" /></svg>")
    });
    await expect(readExamAssetFromDatabase(pool, "inactive-exam", "diagrams/graph.svg")).resolves.toBeNull();
    await expect(
    ...
    await expect(readExamAssetFromDatabase(pool, "inactive-exam", "diagrams/admin-only.svg")).resolves.toBeNull();
    await expect(readExamAssetFromDatabase(pool, "inactive-exam", "diagrams/admin-only.svg", false)).resolves.toMatchObject({
      examId: "inactive-exam",
      path: "diagrams/admin-only.svg"
    });
    ...
    await seedExamCatalog(pool, [{ ...exam, problems: [{ ...exam.problems[0], title: "Updated Problem" }] }], new Set(["db-exam"]));
    ...
    await expect(readExamAssetFromDatabase(pool, "db-exam", "diagrams/graph.svg")).resolves.toBeNull();
  });
});
