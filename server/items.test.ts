...
describe("server items", () => {
    ...
    it("requires an advice-note problem that sender solved and target has not solved", () => {
        ...
        expect(validateItemTarget(room([sender, target]), "adviceNote", sender, target)).toEqual({
            ok: true,
        });
    });
    ...
    it("blocks self-targeting and duplicate active disruptor effects", () => {
        ...
        const target = player({
            id: "target",
            effects: [{ id: "cover", label: "Cover", sourceName: "A", expiresAt: now + 1000 }],
        });

        expect(validateItemTarget(room([sender, target]), "cover", sender, sender, now).ok).toBe(
            false,
        );
        expect(validateItemTarget(room([sender, target]), "cover", sender, target, now).ok).toBe(
            false,
        );
    });
