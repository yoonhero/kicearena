...
export const cleanupEffects = (room: Pick<RoomState, "players">, now = Date.now(), expiredEffectNoticeMs = 3000) => {
  let changed = false;
  for (const player of room.players.values()) {
    ...
    const visibleExpiredEffects = expiredEffects.filter((effect) => now - effect.clearedAt <= expiredEffectNoticeMs);
...
export const findAdviceNoteProblem = (
  ...
  return newest ? room.exam.problems.find((problem) => problem.id === newest.problemId) ?? null : null;
};

export const activeEffectForItem = (target: Pick<PlayerState, "effects">, itemId: ItemId, now = Date.now()) => target.effects.find((effect) => effect.id === itemId && effect.expiresAt > now);

export const validateItemTarget = (room: Pick<RoomState, "exam">, itemId: ItemId, sender: PlayerState, target: PlayerState, now = Date.now()) => {
  ...
  if (item.lifecycle.duplicate === "blockWhileActive" && activeEffectForItem(target, itemId, now)) {
    return { ok: false, error: "대상에게 같은 아이템 효과가 이미 적용 중입니다." } as const;
  }
  return { ok: true } as const;
};

export const leadingScore = (room: Pick<RoomState, "players">) => Math.max(0, ...[...room.players.values()].map((player) => player.score));

export const randomItem = (random = Math.random): ItemId => ITEM_IDS[Math.floor(random() * ITEM_IDS.length)];

export const maybeAwardItems = (room: Pick<RoomState, "itemEnabled" | "players">, player: PlayerState, problem: ProblemManifest, attempts: number, random = Math.random): ItemAward[] => {
  ...
  if (random() < chance) {
    awards.push({ itemId: randomItem(random), reason: comebackBoost > 0.1 ? "comeback" : problem.difficulty >= 4 ? "difficulty" : "lucky" });
  }
