...
- Ops: macOS launchd home-server app, Docker Compose for Postgres/Redis, archived Docker blue/green stack
...
같은 room code의 상태 변경은 advisory lock과 in-process mutex로 직렬화한다. 현재 홈서버 운영에서는 앱을 macOS host의 Bun 프로세스로 실행하고, Postgres/Redis만 Docker Compose로 실행한다. 이전 blue/green app container와 Nginx gateway 구조는 클라우드 전환 전까지 보존 문서로 유지한다.
...
- `HOST_PORT` / `PORT`: host app port, 기본 `3001`
- `KICE_DOCKER_BACKEND`: Postgres/Redis용 Docker backend. Colima를 강제하려면 `colima`
...
- `ADMIN_BASIC_AUTH_HTPASSWD`, `GRAFANA_ADMIN_PASSWORD`, `DISCORD_WEBHOOK_URL`: 보존된 Docker gateway/monitoring stack을 다시 켤 때 사용
...
현재 홈서버 배포는 Docker image를 만들거나 pull하지 않는다. 앱은 macOS host에서 Bun + launchd로 실행하고, Postgres/Redis만 Docker Compose로 실행한다.

```bash
scripts/deploy-home-bun.sh --branch main
```

이 스크립트는 기존 Docker app/gateway 컨테이너를 멈추고, `postgres`/`redis`를 띄운 뒤 `bun install --frozen-lockfile`, `bun run build`, `bun run db:seed`를 실행한다. 이후 `.deploy/run-home-server.sh`와 `~/Library/LaunchAgents/dev.yoonhero.kice-arena.plist`를 쓰고 앱 healthcheck를 확인한다.

GitHub Actions의 `Deploy Home Server` 워크플로도 같은 스크립트를 호출한다. Secrets/Variables를 원격 서버 `.env`로 쓴 뒤 branch를 fast-forward하고 host Bun 앱을 재시작한다.

자세한 절차는 `docs/home-server-bun-deploy.md`에 있다. 이전 Docker image + blue/green gateway 배포 구조는 `docs/docker-blue-green-deploy-archive.md`에 보존했다.

## 모니터링

운영 메트릭과 알림 문서는 `deploy/monitoring/README.md`에 있다. 단, bundled Prometheus/Grafana compose 구성은 이전 Docker gateway 기준이므로 host Bun 배포에서는 scrape target을 조정하기 전까지 기본으로 올리지 않는다. 기본 대시보드는 다음을 본다.
