...
const cloneBody = (body: ProblemBodyBlock[] | undefined): ProblemBodyBlock[] => JSON.parse(JSON.stringify(body ?? [])) as ProblemBodyBlock[];

export const makeDefaultChoices = () => Array.from({ length: DEFAULT_CHOICE_COUNT }, (_, index) => String(index + 1));

const paragraphToMarkup = (block: Extract<ProblemBodyBlock, { kind: "paragraph" }>) => {
  if (!block.inlineMath?.length) return block.text;
  const parts = block.text.split("{}");
  return parts.map((part, index) => `${part}${block.inlineMath?.[index] ? `$${block.inlineMath[index]}$` : ""}`).join("");
};
...
const parseParagraphMarkup = (text: string): Extract<ProblemBodyBlock, { kind: "paragraph" }> => {
  ...
  return inlineMath.length ? { kind: "paragraph", text: normalizedText, inlineMath } : { kind: "paragraph", text };
};
...
export const bodyToMarkup = (body: ProblemBodyBlock[] | undefined) =>
  cloneBody(body)
    .map((block) => {
      ...
      if (block.kind === "diagram") return `::svg ${[block.src, block.alt, block.caption].filter((part) => part !== undefined && part !== "").join(" | ")}`;
...
export const parseProblemMarkup = (markup: string): ProblemBodyBlock[] => {
  ...
  markup.split(/\r?\n/).forEach((rawLine) => {
    ...
    if (command.startsWith("::svg ")) {
      flushParagraph();
      const [src = "", alt = "도표", caption = ""] = command.slice("::svg ".length).split("|").map((part) => part.trim());
      if (src) blocks.push({ kind: "diagram", src, alt: alt || "도표", caption: caption || undefined });
      return;
    }
...
export const normalizedBodyForSave = (form: ProblemFormDraft): ProblemBodyBlock[] => {
  ...
  const body = form.answerKind === "choice" ? parsed : parsed.filter((block) => block.kind !== "choices");
...
export const appendDefaultChoiceMarkup = (markup: string) => {
  const choices = makeDefaultChoices().map((choice) => `::choice ${choice}`).join("\n");
  return `${markup.trim() ? `${markup.trim()}\n\n` : ""}${choices}`;
};

export const assetPathForUrl = (assetPath: string) => assetPath.split("/").map((part) => encodeURIComponent(part)).join("/");
...
export const bodyForAdminPreview = (examId: string, body: ProblemBodyBlock[]) =>
  body.map((block) => (block.kind === "diagram" ? { ...block, src: adminAssetSrc(examId, block.src) } : block));

export const insertMarkupAtRange = (source: string, markup: string, start = source.length, end = start) => {
