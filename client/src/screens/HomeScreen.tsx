...
export function HomeScreen(props: {
    ...
    const effectiveItemEnabled = props.itemEnabled;
    useEffect(() => {
        ...
        <main className={`home-layout ${props.inviteMode ? "invite-home-layout" : ""}`}>
            <section className="exam-sheet intro-sheet omr-entry-sheet">
                ...
                <div className="omr-entry">
                    <div
                        className={`identity-card entry-zone entry-zone-identity ${nameEditorOpen || props.inviteMode ? "name-editor-open" : ""}`}
                    >
                        ...
                        <div className="omr-name-maker" aria-label="성명 OMR 입력">
                            <div className="omr-maker-head">
                                <strong>성명</strong>
                                {props.inviteMode && (
                                    <span>{`초대 방 ${props.inviteRoomCode}`}</span>
                                )}
                            </div>
                            <div
                                className="omr-maker-cells"
                                role="tablist"
                                aria-label="수정할 이름 글자 선택"
                            >
                            ...
                            <div
                                className="omr-syllable-row"
                                aria-label={`${activeSlot + 1}글자 초성 선택`}
                            >
                                {NICKNAME_INITIALS.map((jamo) => (
                                    <button
                                        ...
                                        type="button"
                                        className={
                                            nameParts[activeSlot].initial === jamo ? "marked" : ""
                                        }
                            ...
                            <div
                                className="omr-syllable-row"
                                aria-label={`${activeSlot + 1}글자 중성 선택`}
                            >
                                {NICKNAME_VOWELS.map((jamo) => (
                                    <button
                                        ...
                                        type="button"
                                        className={
                                            nameParts[activeSlot].vowel === jamo ? "marked" : ""
                                        }
                            ...
                            <div
                                className="omr-syllable-row"
                                aria-label={`${activeSlot + 1}글자 종성 선택`}
                            >
                                {NICKNAME_FINALS.map((jamo) => (
                                    <button
                                        ...
                                        type="button"
                                        className={
                                            (nameParts[activeSlot].final ?? "") === jamo
                                                ? "marked"
                                                : ""
                                        }
                        ...
                        <label className="nickname-direct-field">
                            ...
                            <input
                                ...
                                onChange={(event) =>
                                    props.setNickname(sanitizeNickname(event.target.value))
                                }
                                placeholder="닉네임"
                            />
                        </label>
                    ...
                    {!props.inviteMode && (
                        <div className="entry-flow-stack">
                                        ...
                                        >
                                            <span className="exam-cover-art">
                                                ...
                                                <small>
                                                    ...
                                                    {Math.round(exam.timeLimitSec / 60)}분
                                                </small>
                                            </span>
                                        </button>
                                    ))}
                            ...
                            {entryMode === "create" ? (
                                <div
                                    className={`entry-action-panel creator-panel entry-zone entry-zone-action ${showOptions ? "options-open" : ""}`}
                                >
                                    ...
                                    <div
                                        className="quick-preset-list"
                                        role="radiogroup"
                                        aria-label="시험 시간 프리셋"
                                    >
                                        <span>시간 선택</span>
                                            ...
                                            {QUICK_PRESETS.map((preset) => (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        props.setFreezeBeforeMin(
                                                            preset.freezeBeforeMin,
                                                        );
                                                    }}
                                                    role="radio"
                                                    aria-checked={
                                    ...
                                    <div
                                        className={`room-option-drawer ${showOptions ? "open" : ""}`}
                                    >
                                        <button
                                        ...
                                        {showOptions && (
                                            ...
                                            >
                                                <div className="timeline-status" aria-hidden="true">
                                                    <strong>
                                                        {freezeStartMin}분부터 프리즈 ·{" "}
                                                        {props.timeLimitMin}분 종료
                                                    </strong>
                                                </div>
                                                <div className="coupled-time-track">
                                                    <div className="coupled-slider-stack">
                                                        <div
                                                            className="coupled-axis"
                                                            aria-hidden="true"
                                                        />
                                                        <div
                                                            className="freeze-zone"
                                                            aria-hidden="true"
                                                        />
                                                        <div
                                                            className="coupled-ticks"
                                                            aria-hidden="true"
                                                        >
                                                            {timelineTicks.map((minute) => (
                                                                <span
                                                                    key={minute}
                                                                    style={{
                                                                        left: `${(minute / maxTimeLimitMin) * 100}%`,
                                                                    }}
                                                                >
                                                                    {minute}
                                                                </span>
                                                        ...
                                                        <div
                                                            className="axis-arrow left"
                                                            aria-hidden="true"
                                                        />
                                                        <div
                                                            className="axis-arrow right"
                                                            aria-hidden="true"
                                                        />
                                                        <div
                                                            className="time-marker time-limit-marker"
                                                            aria-hidden="true"
                                                        >
                                                            <span>종료</span>
                                                        ...
                                                        <div
                                                            className="time-marker freeze-marker-dot"
                                                            aria-hidden="true"
                                                        >
                                                            <span>프리즈 시작</span>
                                ...
                                <div className="entry-action-panel join-panel entry-zone entry-zone-action">
                                    ...
                                    <div className="omr-field code-field">
                                        ...
                                        <input
                                            ...
                                            onChange={(event) =>
                                                props.setRoomCode(event.target.value.toUpperCase())
                                            }
                                            placeholder="ABCDE"
                                        />
                                    </div>
                                    <button
                                        ...
                                        disabled={!trimmedNickname || !trimmedRoomCode}
