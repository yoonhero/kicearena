...
export function ArenaScreen({
    ...
    const submissionKeyRef = useRef<{ problemId: string; answer: string; key: string } | null>(
        null,
    );
    const timeLeft = useCountdown(room);
    const currentProblem =
        room.exam.problems.find((problem) => problem.id === ownPlayer.currentProblemId) ??
        room.exam.problems[0];
    const hasEffect = (id: string) =>
        ownPlayer.effects.some((effect) => effect.id === id && effect.expiresAt > Date.now());
    ...
    const submit = useCallback(
        async (selectedAnswer = answer) => {
            ...
            const idempotencyKey =
                existingKey?.problemId === currentProblem.id &&
                existingKey.answer === selectedAnswer
                    ? existingKey.key
                    : makeIdempotencyKey();
            ...
            if (response.data?.correct) {
                const awards =
                    response.data.itemAwards ??
                    (response.data.itemAwarded
                        ? [{ itemId: response.data.itemAwarded, reason: "lucky" as const }]
                        : []);
    ...
    useEffect(() => {
        const onKeyDown = (event: KeyboardEvent) => {
            ...
            if (currentProblem.answerKind === "choice") {
                const choiceCount =
                    currentProblem.body?.find((block) => block.kind === "choices")?.choices
                        .length ?? 5;
    ...
    const useItem = async (itemId: ItemId, targetPlayerId: string) => {
        ...
        if (item.payload?.message) {
            const draft = window.prompt(
                item.payload.message.prompt,
                item.payload.message.defaultText,
            );
    ...
    if (view === "rankings") {
        return (
            <RankingsScreen room={room} ownPlayer={ownPlayer} onBack={() => setView("problem")} />
        );
    }
