...
describe("admin problem body markup", () => {
  it("parses markdown-like body lines into supported problem blocks", () => {
    expect(
      ...
      { kind: "paragraph", text: "두 실수 {}와 {}에 대하여\n다음 조건을 만족한다.", inlineMath: ["x", "y"] },
  ...
  it("removes choices from short-answer saves and creates default choices for choice saves", () => {
    ...
    ).toEqual([{ kind: "paragraph", text: "값을 고르시오." }, { kind: "choices", choices: ["1", "2", "3", "4", "5"] }]);
  });

  it("updates choice markup only when the answer type actually needs it", () => {
    expect(removeChoiceMarkup("본문\n\n::choice 1\n::choice 2\n\n::note 끝")).toBe("본문\n\n::note 끝");
    expect(appendDefaultChoiceMarkup("본문")).toBe("본문\n\n::choice 1\n::choice 2\n::choice 3\n::choice 4\n::choice 5");
  });

  it("replaces the selected body range with uploaded asset markup", () => {
    const source = "첫 문단\n\n교체 대상\n\n마지막 문단";
    const start = source.indexOf("교체 대상");
    const result = insertMarkupAtRange(source, "::svg diagrams/uploaded.svg | 그래프", start, start + "교체 대상".length);
  ...
  it("maps relative diagram assets to admin URLs for preview without rewriting absolute sources", () => {
    expect(adminAssetSrc("mock exam", "diagrams/a b.svg")).toBe("/api/admin/exams/mock%20exam/assets/diagrams/a%20b.svg");
    expect(adminAssetSrc("mock-exam", "/api/exams/mock-exam/assets/diagrams/a.svg")).toBe("/api/exams/mock-exam/assets/diagrams/a.svg");
    expect(bodyForAdminPreview("mock-exam", [{ kind: "diagram", src: "diagrams/a.svg", alt: "도표" }])).toEqual([
      { kind: "diagram", src: "/api/admin/exams/mock-exam/assets/diagrams/a.svg", alt: "도표" }
    ]);
  });
});
