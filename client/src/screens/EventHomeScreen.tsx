    ...
    () =>
      events.map((event) => {
        ...
        return {
          ...event,
          startLabel: startsAt
            ? startsAt.toLocaleString("ko-KR", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })
            : "상시 오픈",
      ...
      <main className="gym-layout">
        <section className="gym-entry-panel">
          ...
          <label className="gym-field">
            <span>표시 이름</span>
            <input value={nickname} onChange={(event) => setNickname(event.target.value)} maxLength={6} />
          </label>
          <div className="gym-actions">
            <button type="button" onClick={() => void joinInviteRoom()} disabled={joiningInvite}>
              <LogIn size={18} />
              {joiningInvite ? "입장 중" : "입장"}
            </button>
        ...
        <div className="gym-account-row">
          ...
          <label className="gym-field">
            ...
            <input
              ...
              onChange={(event) => setRegistrationInviteCode(event.target.value.trim())}
...
function EventRow({
  ...
  const registerDisabled = !accountId || !inviteCode || event.status !== "open" || Boolean(pendingEventAction);
  const disabledReason = !accountId
    ? "계정이 없으면 문제 보기만 가능합니다."
    : !inviteCode
      ? "초대 코드가 있어야 등록할 수 있습니다."
      : event.status !== "open"
        ? "아직 등록을 시작하지 않은 이벤트입니다."
        : pendingEventAction
          ? "처리 중입니다."
        : undefined;
  ...
  return (
    <article className="gym-event-row">
      ...
      <div className="gym-event-actions">
        ...
        >
          <Eye size={18} />
          {pendingThisEvent && pendingEventAction?.action === "spectate" ? "불러오는 중" : "공개 문제 보기"}
        </button>
        <button
          type="button"
          className="primary"
        ...
        >
          <UserRound size={18} />
          {pendingThisEvent && pendingEventAction?.action === "register" ? "등록 중" : "초대로 등록"}
        </button>
