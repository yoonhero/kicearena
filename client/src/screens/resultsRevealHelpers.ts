...
export function makeNextRevealLabel(event: RevealEvent | undefined, room: RoomPublic) {
    ...
    const problemNumber =
        room.exam.problems.find((problem) => problem.id === event.submission.problemId)?.number ??
        "?";
...
export function getRevealProblemScrollLeft(board: HTMLElement, target: HTMLElement) {
    ...
    const centeredLeft =
        target.offsetLeft - stickyWidth - (availableWidth - target.offsetWidth) / 2;
    ...
    const visibleLeft = Math.min(
        fullyVisibleLeftLimit,
        Math.max(fullyVisibleRightLeft, centeredLeft),
    );
...
export function findNearestRevealProblemHeader(board: HTMLElement) {
    ...
    const visibleCenter =
        board.scrollLeft + stickyWidth + Math.max(0, board.clientWidth - stickyWidth) / 2;
    return headers.reduce((nearest, header) => {
        ...
        return Math.abs(headerCenter - visibleCenter) < Math.abs(nearestCenter - visibleCenter)
            ? header
            : nearest;
    }, headers[0]);
}
