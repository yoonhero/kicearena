...
export function ResultsScreen({ room, ownPlayer, onLeave }: { room: RoomPublic; ownPlayer: PlayerPublic | null; onLeave: () => Promise<void> }) {
  ...
  const processRevealQueue = useCallback(() => {
    if (revealQueueTimerRef.current !== null || revealInFlightRef.current || reportOpenRef.current) return;
  ...
  const requestRevealAdvance = useCallback(() => {
    ...
    const alreadyRequested = queuedRevealRequestsRef.current + (revealInFlightRef.current ? 1 : 0);
    const remaining = Math.max(0, revealTotalRef.current - revealCountRef.current - alreadyRequested);
    if (remaining <= 0) {
  ...
  useEffect(() => {
    ...
    if (allRevealed && openReportWhenQueueSettlesRef.current && queuedRevealRequestsRef.current === 0 && !revealInFlightRef.current) {
      openReportWhenQueueSettlesRef.current = false;
      setReportOpen(true);
      return;
    }
  ...
  useEffect(() => {
    return () => {
      if (revealQueueTimerRef.current !== null) window.clearTimeout(revealQueueTimerRef.current);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key !== "Enter" || event.repeat || event.defaultPrevented || event.metaKey || event.ctrlKey || event.altKey || event.isComposing || isEditableShortcutTarget(event.target)) return;
      if (!isHost) return;
  ...
  return <FinalReportView room={room} ownPlayer={ownPlayer} players={players} onLeave={onLeave} />;
}
