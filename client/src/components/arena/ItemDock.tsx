...
export function ItemDock({
    ...
    const groupedInventory = useMemo(
        () =>
            ownPlayer.inventory
                .reduce<Array<{ itemId: ItemId; count: number }>>((groups, itemId) => {
    ...
    const suggestedTargetId = useMemo(
        () =>
            selectedItem && room.players.length > 1
                ? standings
                      .filter((standing) => standing.playerId !== ownPlayer.id)
                      .sort(compareStandings)[0]?.playerId
                : null,
    ...
    const maxScore = useMemo(
        () => Math.max(1, ...room.players.map((player) => player.score)),
        [room.players],
    );
    ...
    const canReceiveAdviceNote = (player: PlayerPublic) => {
        ...
        const solvedByTarget = new Set(
            player.submissionHistory
                .filter((submission) => submission.correct)
                .map((submission) => submission.problemId),
        );
    ...
    return (
        <section className={`item-dock ${selectedItem ? "aiming" : ""}`}>
            ...
            <div className="item-bank" aria-label="보유 아이템">
                {ownPlayer.inventory.length === 0 ? (
                    <span
                        ...
                        title="고난도·첫 풀이·추격 상황에서 지급 확률 상승"
                    >
                        <Zap size={16} />
                    </span>
                ) : (
                    groupedInventory.map(({ itemId, count }) => {
                        ...
                        return (
                            <button
                                ...
                                title={
                                    cooldownActive
                                        ? `${item.name}: 재사용 대기 ${formatEffectSeconds(cooldownReadyAt)}`
                                        : `${item.name}: ${item.description} 클릭 후 대상을 선택합니다.`
                                }
                                aria-pressed={selectedItem === itemId}
                            >
                                <ItemIcon itemId={itemId} size={20} />
                                <span>{item.shortName}</span>
                                {(cooldownActive || count > 1) && (
                                    <em>
                                        {cooldownActive
                                            ? formatEffectSeconds(cooldownReadyAt)
                                            : `x${count}`}
                                    </em>
                                )}
                            </button>
            ...
            {selectedItemDefinition && (
                <div
                    className="target-bank"
                    aria-label={`${selectedItemDefinition.name} 적용 대상`}
                >
                    <div className="target-bank-head">
                        ...
                        <button
                            ...
                            aria-label="아이템 선택 취소"
                        >
                            <X size={16} />
                        </button>
                    </div>
                    {targets.map((player) => {
                        const activeEffects = player.effects.filter(
                            (effect) => effect.expiresAt > Date.now(),
                        );
                        const sameEffect = activeEffects.find(
                            (effect) => effect.id === selectedItemDefinition.id,
                        );
                        const expiredEffect = player.expiredEffects.find(
                            (effect) => effect.id === selectedItemDefinition.id,
                        );
                        const selfBlocked =
                            selectedItemDefinition.lifecycle.target === "opponent" &&
                            player.id === ownPlayer.id;
                        const adviceUnavailable =
                            selectedItemDefinition.lifecycle.target === "eligibleUnsolved" &&
                            !canReceiveAdviceNote(player);
                        const duplicateBlocked =
                            selectedItemDefinition.lifecycle.duplicate === "blockWhileActive" &&
                            Boolean(sameEffect);
                        ...
                        return (
                            <button
                                ...
                                style={
                                    {
                                        "--target-score-size": `${scoreSize}px`,
                                    } as React.CSSProperties & Record<string, string>
                                }
                            >
                                <span
                                    className="target-score-orb"
                                    aria-label={`${player.score}점`}
                                />
