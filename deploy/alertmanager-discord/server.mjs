...
const makeDiscordPayload = (payload) => {
    ...
    const group =
        payload.groupLabels?.alertname ?? payload.commonLabels?.alertname ?? "KICE Arena alerts";
    ...
    return {
        ...
        embeds: alerts.slice(0, 10).map((alert) => ({
            ...
            fields: [
                alert.labels?.cause
                    ? { name: "cause", value: truncate(alert.labels.cause, 1024), inline: true }
                    : null,
                alert.labels?.severity
                    ? {
                          name: "severity",
                          value: truncate(alert.labels.severity, 1024),
                          inline: true,
                      }
                    : null,
