...
export const isProblemBodyBlock = (value: unknown): value is ProblemBodyBlock => {
  ...
  if (block.kind === "paragraph") {
    return typeof block.text === "string" && (block.inlineMath === undefined || isStringArray(block.inlineMath));
  }
...
export const isProblemBody = (value: unknown): value is ProblemBodyBlock[] => Array.isArray(value) && value.every(isProblemBodyBlock);
...
export type ItemEffectKind = "screenCover" | "problemRotate" | "hardProblemGate" | "memeOverlay" | "inputLock" | "bannedSong" | "auraOverlay" | "adviceNote";
...
const defineItems = <TItems extends { [TItemId in ItemId]: ItemDefinitionFor<TItemId> }>(items: TItems) => items;
