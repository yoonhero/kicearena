...
const sampleValue = (
    samples: ReturnType<typeof runtimeMetricSamples>,
    name: string,
...
const metricValue = (name: string, labels?: Record<string, string>) => {
    return (summary: ReturnType<typeof summarizeRoomMetrics>) =>
        sampleValue(runtimeMetricSamples(summary), name, labels);
};
...
describe("summarizeRoomMetrics", () => {
    ...
    it("combines playing finish deadlines and cleanup deadlines for avg/max room expiry", () => {
        ...
        expect(
            metricValue("kice_arena_players_per_active_room", { state: "connected" })(summary),
        ).toBe(0.5);
    });
    ...
    it("surfaces stale rooms that should already have been cleaned up", () => {
        ...
        expect(
            metricValue("kice_arena_room_expiry_overdue_seconds", { stat: "max" })(summary),
        ).toBe(15);
    });
...
describe("runtimeMetricSamples", () => {
    it("always emits base room metrics and heartbeat samples, even when there are no rooms", () => {
        ...
        const samples = runtimeMetricSamples(summary, {
            collectedAtMs: now,
            service: "kice-arena-test",
        });

        expect(
            sampleValue(samples, "kice_arena_runtime_metrics_info", { service: "kice-arena-test" }),
        ).toBe(1);
        expect(
            sampleValue(samples, "kice_arena_runtime_metrics_last_success_unixtime", {
                service: "kice-arena-test",
            }),
        ).toBe(1000);
    ...
    it("keeps base and derived sample groups available separately", () => {
        ...
        expect(
            baseRuntimeMetricSamples(summary).some(
                (sample) => sample.name === "kice_arena_rooms_active",
            ),
        ).toBe(true);
        expect(
            derivedRuntimeMetricSamples(summary).some(
                (sample) => sample.name === "kice_arena_rooms_active",
            ),
        ).toBe(false);
        expect(
            runtimeMetricSamples(summary).some(
                (sample) => sample.name === "kice_arena_rooms_active",
            ),
        ).toBe(true);
        expect(
            runtimeMetricSamples(summary).some(
                (sample) => sample.name === "kice_arena_room_disconnect_risk_score",
            ),
        ).toBe(true);
    });
    ...
    it("emits contest event metrics separately from casual room metrics", () => {
        ...
        expect(
            sampleValue(samples, "kice_arena_contest_sessions", {
        ...
        expect(
            sampleValue(samples, "kice_arena_contest_participants", {
                event_id: "preliminary-day",
                state: "total",
            }),
        ).toBe(3);
