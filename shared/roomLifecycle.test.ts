...
describe("room lifecycle policy", () => {
    ...
    it("allows host to kick another lobby player", () => {
        expect(validateLobbyKick(lobbyRoom, "host", "guest")).toEqual({
            ok: true,
    ...
    it("rejects non-host, self, and non-lobby kicks", () => {
        expect(validateLobbyKick(lobbyRoom, "guest", "host")).toEqual({
            ok: false,
            error: "not-host",
        });
        expect(validateLobbyKick(lobbyRoom, "host", "host")).toEqual({
            ok: false,
            error: "self-target",
        });
        expect(validateLobbyKick(playingRoom, "host", "guest")).toEqual({
            ok: false,
            error: "not-lobby",
        });
    });
    ...
    it("closes a room only when no players remain connected", () => {
        expect(
            shouldCloseRoomForNoConnectedPlayers([{ connected: false }, { connected: false }]),
        ).toBe(true);
        expect(
            shouldCloseRoomForNoConnectedPlayers([{ connected: false }, { connected: true }]),
        ).toBe(false);
    });
});
