...
export function makeScoreboardRevealState(
    room: RoomPublic,
    frozenRows = room.frozenStandings,
    appliedCount = 0,
    options: RevealStateOptions = {},
): RevealState {
    ...
    const orderedFrozenRows =
        frozenRows.length > 0
            ? [...frozenRows].sort(compareStandings)
            : makePlayerStandingRows(room);
    ...
    while (hasHiddenSubmission(boardIds, hiddenByPlayer, revealedCounts)) {
        ...
        const rankedIds = [...boardIds].sort((a, b) =>
            compareStandings(
                makePartialStanding(a, orderedFrozenRows, hiddenByPlayer, revealedCounts),
                makePartialStanding(b, orderedFrozenRows, hiddenByPlayer, revealedCounts),
            ),
        );
    ...
    for (const event of events.slice(0, appliedCount)) {
        ...
        replayBoardIds = [...replayBoardIds].sort((a, b) =>
            compareStandings(
                makePartialStanding(a, orderedFrozenRows, hiddenByPlayer, replayCounts),
                makePartialStanding(b, orderedFrozenRows, hiddenByPlayer, replayCounts),
            ),
        );
    }
    ...
    const rows: RevealBoardRow[] = replayBoardIds.map((playerId, index) => {
        const hidden = hiddenByPlayer.get(playerId) ?? [];
        const revealedCount =
            playerId === options.viewerPlayerId ? hidden.length : (replayCounts.get(playerId) ?? 0);
        const rowCounts = new Map(replayCounts);
        if (playerId === options.viewerPlayerId) rowCounts.set(playerId, hidden.length);
        const standing = makePartialStanding(
            playerId,
            orderedFrozenRows,
            hiddenByPlayer,
            rowCounts,
...
function hasHiddenSubmission(
    playerIds: string[],
    hiddenByPlayer: Map<string, SubmissionPublic[]>,
    revealedCounts: Map<string, number>,
) {
    return playerIds.some(
        (playerId) =>
            (revealedCounts.get(playerId) ?? 0) < (hiddenByPlayer.get(playerId)?.length ?? 0),
    );
}

function findBottomRevealPlayer(
    playerIds: string[],
    hiddenByPlayer: Map<string, SubmissionPublic[]>,
    revealedCounts: Map<string, number>,
) {
    return (
        [...playerIds]
            ...
            .find(
                (playerId) =>
                    (revealedCounts.get(playerId) ?? 0) <
                    (hiddenByPlayer.get(playerId)?.length ?? 0),
            ) ?? null
    );
}

function makePartialStanding(
    playerId: string,
    frozenRows: StandingPublic[],
    hiddenByPlayer: Map<string, SubmissionPublic[]>,
    revealedCounts: Map<string, number>,
): StandingPublic {
    const frozen = frozenRows.find((row) => row.playerId === playerId);
    const revealed = (hiddenByPlayer.get(playerId) ?? []).slice(
        0,
        revealedCounts.get(playerId) ?? 0,
    );
    ...
    const lastAcceptedAt = accepted.reduce<number | null>(
        (latest, submission) =>
            latest === null || submission.submittedAt > latest ? submission.submittedAt : latest,
        frozen?.lastAcceptedAt ?? null,
    );
    ...
    return {
        ...
        score:
            (frozen?.score ?? 0) +
            accepted.reduce((sum, submission) => sum + submission.scoreAwarded, 0),
        penaltyMs:
            (frozen?.penaltyMs ?? 0) +
            revealed.reduce((sum, submission) => sum + submission.penaltyMs, 0),
        solved: (frozen?.solved ?? 0) + accepted.length,
        lastAcceptedAt,
    };
}

function makeAttemptLabel(
    ...
    return [...problemById.entries()]
        ...
        .map(([problemId]) => {
            const visible = submissions
                .filter(
                    (submission) =>
                        ...
                        (frozenAt === null || submission.submittedAt <= frozenAt),
                )
                .sort((a, b) => a.submittedAt - b.submittedAt)
                .at(-1);
            const hiddenForProblem = hidden.filter(
                (submission) => submission.problemId === problemId,
            );
            const revealedForProblem = hidden
            ...
            const displayedPenaltyMs = [
                ...
                ...revealedForProblem,
            ].reduce((sum, submission) => sum + submission.penaltyMs, 0);
            ...
            if (latestRevealed) {
                return {
                    ...
                    secondary:
                        remaining > 0
                            ? `${revealedForProblem.length}/${hiddenForProblem.length} 공개`
                            : latestRevealed.correct
                              ? formatPenalty(displayedPenaltyMs)
                              : `${revealedForProblem.length}회 공개`,
                };
            }
